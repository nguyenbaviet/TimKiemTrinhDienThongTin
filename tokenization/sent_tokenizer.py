...
class CrfTokenizer(BaseTokenizer):
    def __init__(self, config_root_path="tokenization/", bi_grams_path='bi_grams.txt', tri_grams_path='tri_grams.txt',
...
def test_base():
    params = load_crf_config('tokenization/crf_config.txt')
    print(params)
    ...
    print(fdict)
    test_sent = "ơ kìa sao lại ngu thế này"
    crf_tokenizer_obj.train('data/tokenized/samples/training/data.txt')
...
def word_tokenizer(sent):
    crf_tokenizer_obj = CrfTokenizer()
    crf_tokenizer_obj.train('data/tokenized/samples/training')
    tokens = crf_tokenizer_obj.tokenize(sent)
    return tokens
